...
- [Deployment overview for Microsoft Foundry Models](https://learn.microsoft.com/azure/ai-foundry/concepts/deployments-overview)
...
Note :  If you cant see API Keys run the following command 


Set-AzCognitiveServicesAccount -ResourceGroupName "   " -Name "   " -DisableLocalAuth $false in cloud shell to see API keys

az cognitiveservices account show --name "  " --resource-group "   " --query "properties.disableLocalAuth"
...
The next step is to connect the resources that you deployed in the prior task to your Microsoft Foundry project. This includes connecting the storage account and Application Insights resource that you created in the first task.

<details markdown="block">
<summary><strong>Expand this section to view the solution</strong></summary>

In your Microsoft Foundry project, select the **Management center** option from the left-hand menu.
...
Then, navigate to the storage account that you created in the first task. Select **Microsoft Entra ID** from the dropdown and then select the **Add connection** button associated with this storage account to make the connection. When you are done, select **Close** to close this modal dialog.
...
You will find many of these variables in the Microsoft Foundry project that you created. To find these values, navigate to the **Overview** page and review the **Endpoints and keys** section, as well as the **Project details** section to the right of it.
...
- `AZURE_AI_AGENT_ENDPOINT`: Copy the **Microsoft Foundry project endpoint** when you have selected the **Microsoft Foundry** library.
