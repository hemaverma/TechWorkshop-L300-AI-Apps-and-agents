...
async def send_message(chat_message: ChatMessage):
    ...
    with tracer.start_as_current_span("a2a_chat_message") as span:
        try:
            # Generate session ID if not provided
            session_id = chat_message.session_id or str(uuid.uuid4())
            span.set_attribute("session_id", session_id)
            span.set_attribute("message_length", len(chat_message.message))
            ...
            span.set_attribute("response_complete", response.get('is_task_complete', False))
...
async def stream_message(chat_message: ChatMessage):
    ...
    try:
        with tracer.start_as_current_span("a2a_chat_stream") as span:
            # Generate session ID if not provided
            session_id = chat_message.session_id or str(uuid.uuid4())
            span.set_attribute("session_id", session_id)
            span.set_attribute("message_length", len(chat_message.message))
